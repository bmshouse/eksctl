...
func createBranch(branch string) (string, error) {
	...
	if err := gitWith(params{Args: []string{"clone", "-b", "master", Repository, cloneDir}, Dir: cloneDir, Env: gitSSHCommand()}); err != nil {
		return "", err
	}
	if err := gitWith(params{Args: []string{"checkout", "-b", branch}, Dir: cloneDir, Env: gitSSHCommand()}); err != nil {
		return "", err
	}
	if err := gitWith(params{Args: []string{"push", "origin", branch}, Dir: cloneDir, Env: gitSSHCommand()}); err != nil {
		return "", err
	}
	return cloneDir, nil
}
...
func deleteBranch(branch, cloneDir string) error {
	...
	return gitWith(params{Args: []string{"push", "origin", "--delete", branch}, Dir: cloneDir, Env: gitSSHCommand()})
}
...
func getBranch(branch string) (string, error) {
	...
	if err := gitWith(params{Args: []string{"clone", "-b", branch, Repository, cloneDir}, Dir: cloneDir, Env: gitSSHCommand()}); err != nil {
		return "", err
	}
	return cloneDir, nil
}
...
func gitWith(params params) error {
